...
use core::codec::{PER_FIELD_POSTING_FORMAT_KEY, PER_FIELD_POSTING_SUFFIX_KEY};
use core::index::point_values;
use core::index::term::*;
...
impl FieldNumbersInner {
    ...
    pub fn set_dimensions(
        ...
        if num_bytes > point_values::MAX_NUM_BYTES {
            bail!(IllegalArgument(
                "dimension num_bytes must be <= point_values::MAX_NUM_BYTES".into()
            ));
        }
        if dimension_count > point_values::MAX_DIMENSIONS {
            bail!(IllegalArgument(
