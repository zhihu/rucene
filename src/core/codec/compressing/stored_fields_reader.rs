...
use core::codec::compressing::CompressingStoredFieldsWriter;
    ...
    fn read_field(
        ...
        match bits & TYPE_MASK {
            BYTE_ARR => {
                ...
                input.read_bytes(data.as_mut(), 0, length)?;
            ...
            NUMERIC_INT => {
                visitor.int_field(info, input.read_zint()?)?;
            }
            NUMERIC_FLOAT => {
                visitor.float_field(info, Self::read_zfloat(input)?)?;
            }
            NUMERIC_LONG => {
                visitor.long_field(info, Self::read_tlong(input)?)?;
            }
            NUMERIC_DOUBLE => {
                visitor.double_field(info, Self::read_zdouble(input)?)?;
            }
            _ => {
